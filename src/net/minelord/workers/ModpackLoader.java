	...
	public void run()
	{
		for (String xmlFile : xmlFiles)
		{
			...
			try
			{
				...
				DownloadUtils.downloadToFile(new URL(DownloadUtils.getStaticMinelordLink(xmlFile)), modPackFile);
			}
			catch (IOException e)
			{
			...
			if (modPackStream == null)
			{
				try
				{
					modPackStream = new URL(DownloadUtils.getStaticMinelordLink(xmlFile)).openStream();
				}
				catch (IOException e)
				{
			...
			if (modPackStream != null)
			{
				...
				for (int i = 0; i < modPacks.getLength(); i++)
				{
					...
					try
					{
						ModPack.addPack(new ModPack(modPackAttr.getNamedItem("name").getTextContent(), modPackAttr.getNamedItem("author").getTextContent(), modPackAttr.getNamedItem("version").getTextContent(), modPackAttr.getNamedItem("logo").getTextContent(), modPackAttr.getNamedItem("url")
								.getTextContent(), modPackAttr.getNamedItem("image").getTextContent(), modPackAttr.getNamedItem("dir").getTextContent(), modPackAttr.getNamedItem("mcVersion").getTextContent(), modPackAttr.getNamedItem("serverPack").getTextContent(), modPackAttr.getNamedItem(
								"description").getTextContent(), modPackAttr.getNamedItem("mods") != null ? modPackAttr.getNamedItem("mods").getTextContent() : "", modPackAttr.getNamedItem("oldVersions") != null ? modPackAttr.getNamedItem("oldVersions").getTextContent() : "", modPackAttr
										.getNamedItem("animation") != null ? modPackAttr.getNamedItem("animation").getTextContent() : "", counter, privatePack, xmlFile));
						counter++;
					}
					catch (Exception e)
					{
